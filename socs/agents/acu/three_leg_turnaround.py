...
def gen_three_leg_turnaround(t0, az0, el0, v0, turntime, az_flag, el_flag, point_group_batch, **kwargs):
    ...
    kwargs:
        midpoint_time (float): The time used by the second leg of the turnaround. Defaults to 1 second.
                               This limits the minimum turnaround time to ~2.0 seconds!
        midpoint_velocity (float): The velocity targeted by the beginning/end of the second leg of the turnaround. Defaults to 0 deg/s.
                                   Midpoint acceleration = 2.0 * midpoint_velocity / midpoint_time.
        tr_step_time (float): The step time between points in the turnaround. Defaults to 0.1 seconds (10Hz).
    """
    ...
    # This should probably not enforce a flat time in the future as this limits the minimum turnaround time to 2.0 seconds.
    # For now this has been proven to work at low scan accelerations.
    midpoint_time = kwargs.get('midpoint_time', 1)  # Controls the length of the second leg of the turnaround
    midpoint_velocity = kwargs.get('midpoint_velocity', 0)  # Controls the velocity target the second leg of the turnaround.
    midpoint_acceleration = 2.0 * midpoint_velocity / midpoint_time
    tr_step_time = kwargs.get('tr_step_time', 0.1)  # This gen has to run faster than the constant velocity scan portions.
    ...
    t_target_1 = t_start_1 + (turntime - midpoint_time) / 2  # The first and third legs share the same portion of the turnaround time.
    az_start_1 = az0
    v_start_1 = v0
    v_target_1 = midpoint_velocity * np.sign(v_start_1)
    a_start_1 = 0
    a_target_1 = midpoint_acceleration * -1 * np.sign(v0)
    ...
    t_target_2 = t_start_2 + midpoint_time
    ...
    t_target_3 = t_start_3 + (turntime - midpoint_time) / 2.0
    az_start_3 = azs_2[-1]
    ...
    ts = np.concatenate([ts_1, ts_2, ts_3]) + t0
    azs = np.concatenate([azs_1, azs_2, azs_3])
    vs = np.concatenate([vs_1, vs_2, vs_3])
    ...
    # This function generates a duplicate Trackpoint of the initial condition that is
    # already generated by the `generate_constant_velocity_scan` of the ACU driver.
    # We just throw it away here.
    turnaround_track.pop(0)
...
def _gen_trajectory(t_i, t_f, xn1_i, x0_i, x0_f, x1_i, x1_f, x2_i, x2_f, step_time):
    ...
    ts = np.arange(t_i, t_f, step_time)  # Divide our times into points with step_time spacing
    vs = x0(ts, *A)  # Solve for the velocity at every time using our solved components

    an1 = get_an1(t_i, xn1_i, *A)  # We have to solve for the first antiderivative component to get the positions
    xs = xn1(ts, an1, *A)  # Now solve for the positions at every time
...
def get_an1(t_i, xn1_i, a0, a1, a2, a3, a4, a5):
    """
    Calculates the solution for the parameter An1_i of a polynomial of order five given a t_i and initial xn1_i.

    Where xn1 is the first antiderivative of the function x0.

    A_n1_i is necessary for calculating x_n1 at a later time.

    Args:
        t_i (float): starting time
        xn1_i (float): Generally, the initial condition of the first anti-derivative of the polynomial solution.
                       In the context of generating turnarounds it is the initial position of the turnaround legs.
        a0-a5 (floats): The solved parameters of the order five polynomial from `solve_fifth_polynomial_lin_eqs`.

    Returns:
        an1 (float): The solved -1st parameter of the order five polynomial.
    """

    return xn1_i - (a0 * t_i + (1 / 2) * a1 * t_i**2 + (1 / 3) * a2 * t_i**3 + (1 / 4) * a3 * t_i**4 + (1 / 5) * a4 * t_i**5 + (1 / 6) * a5 * t_i**6)
