...
class PCR500MA:
    ...
    def __init__(self, ip_addr, port=PORT, timeout=TIMEOUT_DEFAULT):
        ...
        self._com.connect((ip_addr, port))
    ...
    def set_volt_ac(self, volt, volt_llim=None, volt_ulim=None):
        ...
        '''
        assert 0 <= volt <= VOLT_ULIM_SOFT, f'Frequency should be within [0, {VOLT_ULIM_SOFT}] V'
        ...
        if (volt_llim is None) and (volt_ulim is None):
            ...
            assert volt_llim <= volt <= volt_ulim, 'Target frequency is not in the given range'
    ...
    # FIXME : implement VOLT:OFFS functionalities
