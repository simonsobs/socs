...
from socs.agent.ethernet_interface import EthernetInterface


class PsuPrologixInterface(PrologixInterface):
    '''
    Class for using the PSU to prologix interface.
    Used for connecting to PSU via GPIB to ethernet converter.
    '''
...
class PsuEthernetInterface(EthernetInterface):
    '''
    Class for using a PSU to ethernet interface.
    Used for connecting to the PSU via direct ethernet connection.
    '''
    def __init__(self, ip_address, port_number, verbose=False, **kwargs):
        self.verbose = verbose
        super().__init__(ip_address, port_number, **kwargs)
