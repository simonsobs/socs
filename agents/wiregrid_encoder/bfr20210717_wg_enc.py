                ...
                if len(self.parser.encoder_queue):
                    ...
                    if len(pru_clock) > NUM_ENCODER_TO_PUBLISH \
                        ...
                        for data_ind in range(int(len(pru_clock)/COUNTER_INFO_LENGTH)):
                            ...
                            rdata['data']['ave_count']       = np.mean(ref_count)
                            self.agent.publish_to_feed('WGEncoder_rough', rdata)

                            if current_time - position_timestamp > 1.:
                                with open('/data/wg-data/position.log', 'a') as f:
                                    f.write(str(received_time_list[-1])+' '+str(ref_count[-1])+'\n')
                                    pass
                                position_timestamp = current_time
                                pass
                            pass
