...
class WGActuatorAgent:

    def __init__(self, agent, ip_adress='192.168.1.100'):
        ...
        self.agent.register_feed('tmp_actuator', record=True, agg_params=agg_params, buffer_time=1)

        #self.g = gclib.py()
        #self.g.GOpen(ip_adress)
        #with open('info_test.txt', 'w') as testf:
        #    testf.write('{}'.format(self.g.GInfo()))
        #    pass
        pass
    ...
    def start_acq(self, session, params=None):
        ...
        with self.lock.acquire_timeout(0, job='upload_cal_curve') as acquired:
            ...
            while self.take_data:
                ...
                self.agent.publish_to_feed('tmp_actuator', data)
        ...
        self.agent.feeds['tmp_actuator'].flush_buffer()
...
if __name__=='__main__':
    ...
    pgroup.add_argument('--ip-address', default='192.168.1.100')
    args = parser.parse_args()

    site_config.reparse_args(args, 'WGActuatorAgent')
    agent, runner = ocs_agent.init_site_agent(args)
    test_agent = WGActuatorAgent(agent, ip_adress=args.ip_address)
