...
def constant_velocity_scanpoints(azpts, el, azvel, acc, ntimes):
    ...
    Parameters:
        ...
        ntimes(int): Number of times to travel between the endpoints.
            ntimes = 1 corresponds to a scan from, ex., left to right, and
            does not return to left.
...
def ptstack_format(conctimes, concaz, concel, concva, concve, az_flags, 
    ...
    Params:
        ...
        az_flags (list): List of flags associated with azimuth motions at
            associated times
        el_flags (list): List of flags associated with elevation motions
            at associated times
        generator (bool): Toggles the start time. When true, start time is
            start_offset, otherwise start time is time.time() + 
            start_offset

    Returns:
        all_lines: List of lines in the correct format to upload to the
            ACU.
    """
    fmt = '%j, %H:%M:%S'
    if generator:
...
def generate_constant_velocity_scan(az_endpoint1, az_endpoint2, az_speed,
    ...
    Params:
        ...
        acc (float): turnaround acceleration for the azimuth motion at th
            endpoints
        el_endpoint1 (float): elevation endpoint at which to start the
            motion
        ...
        num_batches (int or None): sets the number of batches for the
            generator to create. Default value is None (interpreted as
            infinite batches).
        ...
        wait_to_start (float): number of seconds to wait between
            start_time and when the scan actually starts. Default is 3
            seconds.
        step_time (float): time between points on the constant-velocity
            parts of the motion. Default value is 0.1 seconds. Minimum
            value is 0.05 seconds.
        batch_size (int): number of values to produce in each iteration.
            Default is 500. Batch size is reset to the length of one leg
            of the motion if num_batches is not None.
        ptstack_fmt (bool): determine whether values are produced with the
            necessary format to upload to the ACU. If False, this function
            will produce lists of time, azimuth, elevation, azimuth
            velocity, elevation velocity, azimuth flags, and elevation
