...
# https://www.applied-motion.com/sites/default/files/hardware-manuals/Host-Command-Reference_920-0002W.PDF
#
##########################################################################
    ...
    def get_immediate_position(self, motor=ALL, inches=True):
        ...
        **Task** - Get the position of the motor while it is currently in 
        ...
        for motor in (m_list):
            ...
            i_pos = motor.writeread(b'IP\r')
            sleep(0.1)
            motor.flushInput()
            i_pos = int(i_pos.rstrip(b'\r')[3:])
            if inches:
    ...
    def move_axis_to_position(
        ...
        **Task** - Move the axis to the given absolute position in counts or
        ...
        for motor in (m_list):
            ...
            # Move the motor
            motor.write(b'DI%i\r' % (unit_pos))  # DI = Distance/Position
        ...
        **Task** - Set motor enable to true or false for given axis. Should
        ...
        for motor in (m_list):
            if not motor:
                ...
                continue
            if enable:
                motor.write(b'ME\r')  # ME = Motor Enable
            else:
                motor.write(b'MD\r')  # MD = Motor Disable
            motor.flushInput()
        ...
        **Task** - Retrieve all motor step counts to verify movement.
        ...
        for motor in (m_list):
            if not motor:
                print("Specified motor is invalid - no encoder info.")
                continue
            e_pos = motor.writeread(b'EP\r')  # EP = Encoder Position
            sleep(0.1)
            motor.flushInput()
            e_pos = int(e_pos.rstrip(b'\r')[3:])
    ...
    def set_encoder_value(self, motor=ALL, value=0):
        ...
        **Task** - Set the encoder values in order to keep track of absolute 
        ...
        for motor in m_list:
            ...
            e_pos_set = motor.write(b'EP%i\r' % (value))  # EP = Encoder Position
            sleep(0.1)
            motor.flushInput()
