...
class LS336_Agent:
    ...
    def init_lakeshore_task(self, session, params = {}):
        ...
        with self.lock.acquire_timeout(job = 'init', timeout = 0) as acquired:
            ...
            session.add_message(f'Lakeshore initialized with ID: {self.module.id}')

        self.initialized = True

        # start data acq if passed
        if params.get('auto_acquire', False): self.agent.start('acq')
    ...
    def start_acq(self, session, params = {}):
        ...
        with self.lock.acquire_timeout(job = 'acq', timeout = 3) as acquired:
            ...
            while self.take_data:
                ...
                temps = np.array(np.char.split(self.module.get_kelvin('0'), sep = ',').item()).astype(float) # casts to array of floats
                voltages = np.array(np.char.split(self.module.get_sensor('0'), sep = ',').item()).astype(float) # casts to array of floats
                ...
                if not self.lock.acquire(timeout = 10, job = 'acq'):
                    self.log.warn(f"Failed to re-acquire sampling lock, currently held by {self.lock.job}.")
                    continue
    ...
    def set_heater_range(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_heater_range', timeout = 3) as acquired:
            ...
            session.add_message(f"Set {heater.output_name} range to {params['range']}")
    ...
    def set_pid(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_pid', timeout = 3) as acquired:
            ...
            session.add_message(f"Set {heater.output_name} PID to {params['P']}, {params['I']}, {params['D']}")
    ...
    def set_mode(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_mode', timeout = 3) as acquired:
            ...
            if params['mode'] == current_mode:
                print('Current heater mode matches commanded value. Proceeding unchanged')
            else:
                heater.set_mode(params['mode'])
                time.sleep(params.get('wait', self.wait))

            session.add_message(f"Set {heater.output_name} mode to {params['mode']}")
    ...
    def set_heater_resistance(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_heater_resistance', timeout = 3) as acquired:
            ...
            session.add_message(f"Set {heater.output_name} resistance to {params['resistance']}")
    ...
    def set_max_current(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_max_current', timeout = 3) as acquired:
            ...
            if params['current'] == current_max_current:
                print('Current max current matches commanded value. Proceeding unchanged')
            else:
                heater.set_max_current(params['current'])
                time.sleep(params.get('wait', self.wait))

            session.add_message(f"Set {heater.output_name} max current to {params['current']}")
    ...
    def set_manual_out(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_manual_out', timeout = 3) as acquired:
            ...
            session.add_message(f"Set {heater.output_name} manual out to {params['percent']}")
    ...
    def set_input_channel(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_input_channel', timeout = 3) as acquired:
            ...
            if params['input'] == current_input_channel:
                print('Current input channel matches commanded value. Proceeding unchanged')
            else:
                heater.set_input_channel(params['input'])
                time.sleep(params.get('wait', self.wait))

            session.add_message(f"Set {heater.output_name} input channel to {params['input']}")
    ...
    def set_T_limit(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'set_T_limit', timeout = 3) as acquired:
            ...
            session.add_message(f"Set {channel.input_name} T limit to {params['T_limit']}")
    ...
    def servo_to_temperature(self, session, params):
        ...
        with self.lock.acquire_timeout(job = 'servo_to_temperature', timeout = 3) as acquired:
            ...
            session.add_message(f'Turning ramp off and setting setpoint to current temperature {current_temp}')
            heater.set_ramp_on_off('off')
...
if __name__ == '__main__':
   ...
   # Create an argument parser
    parser = make_parser()
    args = site_config.parse_args(agent_class = 'Lakeshore336Agent', parser = parser)
    ...
    agent.register_task('init_lakeshore', lake_agent.init_lakeshore_task, startup = init_params)
    agent.register_task('set_heater_range', lake_agent.set_heater_range)
    agent.register_task('set_heater_resistance', lake_agent.set_heater_resistance)
